...
CiphertextVector NNBatched::slice(CiphertextVector ctxt, int idx_begin,
                                      int idx_end) {
...
// return lhs < rhs
std::unique_ptr<seal::Ciphertext> NNBatched::lower(CiphertextVector &lhs,
...
seal::Ciphertext NNBatched::XOR(seal::Ciphertext &lhs,
                                    seal::Ciphertext &rhs) {
...
void NNBatched::print_plaintext(seal::Plaintext &ptxt) {
  // decode plaintext
...
void NNBatched::print_ciphertext(seal::Ciphertext &ctxt) {
  seal::Plaintext ptxt;
...
seal::Plaintext NNBatched::encode(std::vector<uint64_t> numbers,
...
void NNBatched::print_vec(seal::Ciphertext &ctxt) {
  seal::Plaintext p;
...
std::vector<seal::Ciphertext> NNBatched::split_by_binary_rep(
    seal::Ciphertext &ctxt) {
...
void NNBatched::run_nn() {
  ...
  // perform sum & rotate to compute the result of the nn program
...
std::unique_ptr<seal::Ciphertext> NNBatched::multvect(
...
std::unique_ptr<seal::Ciphertext> NNBatched::equal(CiphertextVector &lhs,
                                                       CiphertextVector &rhs) {
