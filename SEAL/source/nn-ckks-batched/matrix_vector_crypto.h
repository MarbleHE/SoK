...
/**
 * \brief Decrypts a ciphertext and compares it to an expected vector of plaintext values.
 * \param[in] ctxt_r Ciphertext to decrypt
 * \param[in] expected vector of "plaintext" (=doubles) values to compare against
 * \param[in] decryptor Used to decrypt
 * \param[in] encoder Used to encode the seal::Plaintext generated by decryption into a vector of doubles
 * \param[in] tolerance Ratio by which values can disagree. Default 0.001, i.e. 0.1%
 */
bool decrypt_and_compare(const seal::Ciphertext& ctxt_r, vec expected, seal::Decryptor& decryptor, seal::CKKSEncoder& encoder, float tolerance = 0.001);